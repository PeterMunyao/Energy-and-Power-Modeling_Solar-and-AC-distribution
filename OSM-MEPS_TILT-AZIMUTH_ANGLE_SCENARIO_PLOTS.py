#PRINT-OUT THE ENERGY VALUES AND GRAPHS
...
num_panels = 2592            # Number of PV panels
...
plt.rcParams["font.family"] = "Garamond"
plt.rcParams.update({'font.size': 14})
plt.rcParams['xtick.labelsize'] = 16  # X-axis tick values
plt.rcParams['ytick.labelsize'] = 16  # Y-axis tick values
...
energy_results = {}  # to hold per-day energy
annual_energy_MWh = {}  # for legend info

# === SIMULATION LOOP ===
for az in azimuth_angles:
    energy_results[az] = {}
    annual_energy_MWh[az] = {}
    ...
    for tilt in tilt_angles:
        ...
        module_temp = noct + poa_total / 800 * (28 - temp_amb)
        panel_power = panel_power_max * (1 + temp_coeff * (module_temp - noct))
        dc_power = panel_power * poa_total / 1000
        ac_power = dc_power * inverter_eff
        total_power = ac_power * num_panels

        # Daily kWh (5 min resolution)
        daily_energy = (total_power * (5 / 60)).resample('D').sum() / 1000
        energy_results[az][tilt] = daily_energy
        annual_energy_MWh[az][tilt] = daily_energy.sum() / 1000  # convert to MWh

# === PLOTTING ===
fig, axes = plt.subplots(len(azimuth_angles), 1, figsize=(15, 17), sharex=True, facecolor='#f9f9f9')

legend_handles = []

for i, az in enumerate(azimuth_angles):
    ax = axes[i]
    ax.set_title(f"Azimuth {az}°", fontsize=17, fontweight='bold')
    ax.set_ylabel("Daily Energy (kWh)", fontsize=18, fontweight='bold', color='black')
    ax.grid(True, linestyle='--', alpha=0.6)

    for tilt in tilt_angles:
        daily_series = energy_results[az][tilt]
        annual_energy = annual_energy_MWh[az][tilt]
        label = f"Tilt {tilt}° ({annual_energy:.2f} MWh)"

        line, = ax.plot(daily_series.index, daily_series, label=label, linewidth=1.6)
        if i == 0:
            legend_handles.append(line)

axes[-1].set_xlabel("Date", fontsize=17, fontweight='bold', color='black')
...
# === SAVE PLOT ===
plt.savefig("IEEE_Tilt_Azimuth_Energy_Analysis_2024.pdf", format='pdf', bbox_inches='tight', dpi=300)
plt.show()
...
for az in azimuth_angles:
    ...
    for tilt in tilt_angles:
        ...
        daily_energy = (total_power * (5 / 60)).resample('D').sum() / 1000  # in kWh
        energy_results[az][tilt] = daily_energy / 1000  # convert to MWh for plotting
        annual_energy_MWh[az][tilt] = daily_energy.sum() / 1000  # total MWh

# === PLOTTING MAIN AXES ===
fig, axes = plt.subplots(len(azimuth_angles), 1, figsize=(15, 22), sharex=True, facecolor='#f9f9f9')

legend_handles = []
for i, az in enumerate(azimuth_angles):
    ax = axes[i]
    ax.set_title(f"Azimuth {az}°", fontsize=18, fontweight='bold')
    ax.set_ylabel("Daily Energy (MWh)", fontsize=18, fontweight='bold', color='black')
    ax.grid(True, linestyle='--', alpha=0.6)

    for tilt in tilt_angles:
        daily_series = energy_results[az][tilt]
        line, = ax.plot(daily_series.index, daily_series, linewidth=2, label=f"Tilt {tilt}°")

        if i == 0:
            legend_handles.append(line)  # Only once, to avoid duplicates


axes[-1].set_xlabel("Date", fontsize=17, fontweight='bold', color='black')

# === SET X-LIMIT TO MATCH FIRST/LAST DATE ===
first_date = min([energy_results[az][tilt].index.min() for az in azimuth_angles for tilt in tilt_angles])
last_date = max([energy_results[az][tilt].index.max() for az in azimuth_angles for tilt in tilt_angles])
for ax in axes:
...
plt.savefig("IEEE_Tilt_Azimuth_Energy_Analysis_2024_FINAL.pdf", format='pdf', bbox_inches='tight', dpi=1000)
plt.show()

#-------------------------------------------------
#KISUMU-KENYA
#TIL-AZIMUTH SCENARIOS
