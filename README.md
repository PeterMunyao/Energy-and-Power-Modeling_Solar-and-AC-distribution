...
In our `simulation code, we used the correct formula`, as can be seen by the files uploaded. However, we mistyped two instances of the equation in our journal paper (+ ought to be · i.e. `sin(θ_z)·sin(θ_t) + cos(φ_s − φ_p) -> sin(θ_z)·sin(θ_t)·cos(φ_s − φ_p))`. We therefore disclose this to readers and modelers in general.
