#VNP Technologies rooftop pv plant model comparison with PVoutput site measured data
import pandas as pd
import numpy as np
...
fig, ax = plt.subplots(figsize=(13, 7), facecolor='#f9f9f9')
ax.set_facecolor('#f9f9f9')

# Plot modeled outputs (no markers)
...
if not pvoutput_actual.empty:
    ...
    ax.plot(measured.index, measured, label="VPN-Technologies Rooftop PV Output Energy (Measured)", linestyle='-', linewidth=3, color='blue')
...
# No title as you didn't ask for it, add if needed:
# ax.set_title("VPN Technologies Rooftop Daily Solar Energy (2024): Modeled vs Measured", fontsize=20)

# Legend with font size 14
ax.legend(fontsize=15.5, loc='lower center', bbox_to_anchor=(0.5, -0.2), ncol=3, frameon=False)
