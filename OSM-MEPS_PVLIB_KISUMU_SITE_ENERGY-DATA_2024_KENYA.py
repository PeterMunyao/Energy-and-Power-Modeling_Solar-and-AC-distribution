...
losses = 0.92   # 8% losses → keep power realistic
stc_irradiance = 1000
latitude = -0.085477
longitude = 34.718851
...
def calculate_shading_factor(timestamp):
    hour = timestamp.hour
    month = timestamp.month

    if month in [9, 10, 11, 12]:
        base_shading = 0.8 #soiling factor model parameter
    else:
        base_shading = 0.9 #soiling factor model parameter

    # Extra shading for morning and evening hours
    if 6 <= hour < 8 or 17 <= hour < 18:
        return base_shading * 0.5   # more shading in morning/evening
    elif 9 <= hour < 16:
        return base_shading * 1   # normal midday shading 
    else:
        return base_shading * 0.5   # early morning / late evening: most shading


shading_factor = df.index.map(calculate_shading_factor)
poa_irradiance *= shading_factor

# Cell temperature and DC/AC power
temp_cell = sapm_cell(poa_irradiance, temp_amb, wind_speed, -3.47, -0.0594, 3)
dc_power_pvlib = poa_irradiance / stc_irradiance * num_panels_simple * panel_power_max * (1 + temp_coeff * (temp_cell - 25))
...
surface_tilt_rad = np.radians(tilt)
...
poa_total_simple = (poa_direct + poa_diffuse + poa_reflected) * shading_factor

shading_factor = df.index.map(calculate_shading_factor)
poa_total_simple *= shading_factor

# Add small humidity correction
humidity_factor = 1 - 0.07 * (df['relative_humidity'] / 100)
poa_total_simple *= humidity_factor

module_temp_simple = 45 + poa_total_simple / 800 * (28 - df['air_temp'])
temp_coeff_simple = -0.0045
panel_power_simple = panel_power_max * (1 + temp_coeff_simple * (module_temp_simple - 45))
dc_power_simple = panel_power_simple * poa_total_simple / 1000
ac_power_simple = dc_power_simple * inverter_efficiency
...
ax.plot(df["Daily_Energy_kWh_pvlib"].dropna(), label="PVLIB Model (dynamic shading)", color='orange', linewidth=2.5)
ax.plot(df["Daily_Energy_kWh_simple"].dropna(), label="OSM-MEPS Model (dynamic shading)", color='green', linewidth=2.5)
ax.plot(real_energy["Generated_kWh"], label="Measured: Kisumu, Kenya", color='blue', linewidth=2.5)
...
ax.legend(fontsize=15, loc='upper center', bbox_to_anchor=(0.5, -0.18), ncol=3, frameon=False)

plt.tight_layout()
plt.savefig("Figure_28.pdf", format="pdf", bbox_inches='tight')
plt.show()
